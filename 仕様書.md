...
| GET | `/api/read` | バッファデータ読み取り（任意で `?jinjya=<id>` 指定可） | なし |
...
**バリデーション規則**:
- 各フィールド最大200文字（tags/extra内のキーおよび値も含む）
- NGワードフィルタリング適用（tags/extra内のキーおよび値も含む）
- Zodスキーマによる型検証
- 神社ID (`jinjya`): 1〜64文字の半角英数字・ハイフン・アンダースコアのみ
- tags: カテゴリ別おみくじ結果（任意）
- extra: 追加情報（任意）
...
**概要**: 1時間以内のバッファデータを各神社のGoogle Sheetsに送信（レート制限あり）

**処理フロー**:
1. 1時間以内のバッファデータを取得（空の場合は200 OK「公開対象のおみくじはありません」を返す）
2. 神社IDごとにグループ化し、キーからタイムスタンプを復元
3. D1から各神社のspreadsheet_url取得
4. Google Sheets WebhookにPOST送信（エラー発生時は詳細な内容を隠蔽）
5. 成功した場合はバッファから削除
...
### 6.3 データ保護
- **CORS**: 全ての `/api/*` エンドポイントでCORSおよびOPTIONSプリフライト（204）対応
...
- 全てのエンドポイント（`/api/jinjya/deregister`, `/api/publish` 含む）にレート制限適用
